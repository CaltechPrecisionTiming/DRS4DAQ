...
void readOutData(struct vmeBusHandles *busHandles1,
		 struct vmeBusHandles *busHandles2,
		 struct commandLineParameters params)
{
  ...
  unsigned int eventSize1;
  unsigned int eventSize2;
  ...
  sprintf(fileName1, "%s-%d-bd1.dat", params.datasetName, params.runNumber);
  printf("Writing data to: %s\n", fileName1);
  fileHandle1 = fopen(fileName1, "w");

  sprintf(fileName2, "%s-%d-bd2.dat", params.datasetName, params.runNumber);
  printf("Writing data to: %s\n", fileName2);
  fileHandle2 = fopen(fileName2, "w");

  if (params.siftData) {
    // snprintf(fileName, 100, "%6s.%06d-sifted", params.datasetName, params.runNumber);
    sprintf(fileName1, "%s-%d-sifted-bd1.dat", params.datasetName, params.runNumber);
    siftedHandle = fopen(fileName1, "w");
  }

  while (eventCount < params.maxTriggers) {

    eventsStored = busHandles1->mappedMemory[V1742_Event_Stored];
    eventSize1 = busHandles1->mappedMemory[V1742_Event_Size];

    while (eventSize1 == 0) {
      if (acqStopped) {
	printf("Buffers empty, starting acquisition...\n");
	acqStopped = 0;
	setReg(busHandles1->mappedMemory, V1742_Acquisition_Mode, 0x000C);
      }

      eventSize1 = busHandles1->mappedMemory[V1742_Event_Stored];
      usleep(10000);
    }
        
    if (params.bufferLimit == 1) {
      if (busHandles1->mappedMemory[V1742_Group_Status] & 0x1 && acqStopped == 0) {
	printf("Buffers full, stopping acquisition...\n");
	acqStopped = 1;
	setReg(busHandles1->mappedMemory, V1742_Acquisition_Mode, 0x0000);
	usleep(100000);
	printf("Event stored: %d\n", busHandles1->mappedMemory[V1742_Event_Stored]);
    ...
    eventSize1 = busHandles1->mappedMemory[V1742_Event_Size];
    eventSize2 = busHandles2->mappedMemory[V1742_Event_Size];

    if (eventCount % 50 == 0){
      printf("Read out triggered: event %d, %d bytes.\n", eventCount, eventSize1 * 4);
      printf("Read out triggered: event %d, %d bytes.\n", eventCount, eventSize2 * 4);
    }
    ...
    while (eventSize1 > 0) {
      ...
      if(eventSize1 > 1023){
	readResult1 = vme_dma_read(busHandles1->busHandle, busHandles1->dmaHandle,
				  0, 0x10000000, VME_A32UD, 0xFFC, 0);

	readResult2 = vme_dma_read(busHandles2->busHandle, busHandles2->dmaHandle,
				  0, 0x20000000, VME_A32UD, 0xFFC, 0);

	fwrite(busHandles1->dmaBufferMemory, sizeof(unsigned int), 1023, fileHandle1);
	fwrite(busHandles2->dmaBufferMemory, sizeof(unsigned int), 1023, fileHandle2);

	if (params.siftData) {
	  siftData(1023, busHandles1->dmaBufferMemory);
	}
	...
	eventData1 = busHandles1->mappedMemory[0];
	fwrite(&eventData1, sizeof(unsigned int), 1, fileHandle1);

	eventData2 = busHandles2->mappedMemory[0];
	fwrite(&eventData2, sizeof(unsigned int), 1, fileHandle2);

	if (params.siftData) {
	  siftData(1, &eventData1);
	}
...
void readOutData1(struct vmeBusHandles *busHandles,
		 struct commandLineParameters params)
{
  FILE *fileHandle;
  char fileName[100];
...
void readOutData2(struct vmeBusHandles *busHandles,
		 struct commandLineParameters params)
{
  FILE *fileHandle;
  char fileName[100];
...
void setupVME1(struct vmeBusHandles *handles)
{
...
void setupVME2(struct vmeBusHandles *handles)
{
  if (vme_init(&handles->busHandle)) {
    printf("Error initializing the bus handle.\n");
    exit(-1);
...
main(int argc, char **argv){
  ...
  struct commandLineParameters params;
  struct vmeBusHandles busHandles1;
  struct vmeBusHandles busHandles2;
  ...
  setupVME1(&busHandles1);
  setupVME2(&busHandles2);

  if(params.status){
    displayStatus(busHandles1.mappedMemory);
    displayStatus(busHandles2.mappedMemory);
  }

  
  if(params.run){
    configureDigitizer(params, &busHandles1);
    configureDigitizer(params, &busHandles2);

    // readOutData1(&busHandles1, params);
    // readOutData2(&busHandles2, params);
    readOutData(&busHandles1, &busHandles2, params);
    
  }
  

  if(params.softwareTrigger){
    triggerDigitizer(&busHandles1);
    triggerDigitizer(&busHandles2);
  }

  tearDownVME(&busHandles1);
  tearDownVME(&busHandles2);
