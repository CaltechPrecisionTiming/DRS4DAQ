...
//   tree->Branch("c8", &c8, "c8[1024]/F");
 ...
 for (int i=0 ; i<drs->GetNumberOfBoards() ; i++) {
    ...
    if(  b1->GetBoardSerialNumber() == 2450) serial_2 = i;
    if(  b1->GetBoardSerialNumber() == 2449) serial_1 = i;
    if(  b1->GetBoardSerialNumber() == 2422) serial_3 = i;
  }
  ...
  for( int i =0 ; i < atoi( argv[4]) ; i++) {
    ...
    //    while (b1->IsBusy() || b2->IsBusy() || b3->IsBusy()); //trigger on board1 AND board2 AND board3
